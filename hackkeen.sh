...
echo "| | | | __ _  ___| | _| |/ /___  ___ _ __    / ___|   _| |__   ___ _ __ "
...
echo "[+] Scanning with Jaeles..."
	jaeles scan -s /home/kali/tools/jaeles-signatures/ -U /home/kali/projects/$domain/domains/probed.txt -o ~/projects/$seed/$domain/vulnerabilities/jaeles

echo "[+] Scanning with ChopChop..."
	chopchop scan -u ~/projects/$seed/$domain/domains/probed.txt -c ~/tools/ChopChop/chopchop.yml --insecure --export-filename ~/projects/$seed/$domain/vulnerability/chopchop | grep -iv error
...
case "${flag}" in
	...
	n) domain=${OPTARG}     #for scanning individual domain for vulnerabilities
		...
		echo "[+] Scanning with Nuclei..."
		nuclei -update-templates
		nuclei -t ~/nuclei-templates/ -l ~/projects/$seed/$domain/domains/probed.txt -o ~/projects/$seed/$domain/vulnerabilities/nuclei
		echo "[+] Scanning with Jaeles..."
		jaeles scan -s /home/kali/tools/jaeles-signatures/ -U /home/kali/projects/$domain/domains/probed.txt -o ~/projects/$seed/$domain/vulnerabilities/jaeles
	;;
        *) echo "Choose flag: "
